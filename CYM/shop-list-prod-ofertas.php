                            ...
                            <div v-for="(item, index) in listaHoja" class="col-md-4 col-6 col-lg-3">
                                <div class="product">
                                    <div class="product_img">
                                        <a :href="'shop-product-detail.php?ofer=cyv&prod='+item.prod_id">
                                    ...
                                    <div class="product_info">
                                        <h6 class="product_title titulo_prod"><a :href="'shop-product-detail.php?ofer=cyv&prod='+item.prod_id">{{item.nombre}}</a></h6>

                                        <div style="font-size: 14px;" class="product_price">
                                            <span class="price">${{formatNumerPrecio(item.precio_oferta)}}</span>
                                            <del>${{formatNumerPrecio(item.precio)}}</del>
                                            <div style="font-size: 11px;" class="on_sale">
                                                <span>Ahorra ${{formatNumerDiferencia(item.precio,item.precio_oferta)}}</span>
                                            </div>
                                        </div>
                                        <div style="font-size: 14px;" class="product_price">
                                            <span class="price">S/. {{getCamBio(formatNumerPrecio(item.precio_oferta))}}</span>
                                            <del>S/. {{getCamBio(formatNumerPrecio(item.precio))}}</del>
...
<script>
    ...
    const APP_PROD = new Vue({
        ...
        methods: {
            ...
            getDataProdListaPag2() {
                ...
                $.ajax({
                    ...
                    success: function(resq) {
                     ...
                     /*    $("#en-stock").attr('checked','checked') */
                        console.log('dede lista pag 2');
            ...
            formatNumerDiferencia(precio1, precio2) {
                return (parseFloat(precio1) - parseFloat(precio2)).toFixed(2);
            },

            agregarCarritoProd(index) {
                const prod = this.listaHoja[index];
                prod.precio = prod.precio_oferta
                /*  console.log(prod.precio_oferta); */
