...
<div class="modal fade subscribe_popup" id="onload-popup" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    if ($rowPRodBan = $conexion->query($sql)->fetch_assoc()) {
        ...
        $dataExtraBann5Pc = '<span class="price">$' . number_format($rowPRodBan['precio_oferta']*1.05, 2, '.', ',') . '</span>
                            <del>$' . $precioSHOP . '</del>';
        $dataExtraBannSol5Pc = '<span class="price">(S/. ' . number_format(($rowPRodBan['precio_oferta']*1.05) * $tc, 2, '.', ',') . ')</span>
                            <del>S/. ' . number_format($data['precio'] * $tc, 2, '.', ',') . '</del>';
    } else {
        if($data['tipo_pro']==2){
            ...
            $dataExtraBannSol5Pc = '<span class="price">(S/. ' . number_format(($data['precio_prod']*1.05) * $tc, 2, '.', ',') . ')</span>';
            $precio = number_format($data['precio_prod'] * $tc, 2, '.', ',');
        }else{
            ...
            $dataExtraBannSol5Pc = '<span class="price">(S/. ' . number_format(($data['precio'] *1.05)* $tc, 2, '.', ',') . ')</span>';
                        ...
                        <div class="pr_detail">
                            <div class="product_description">
                                ...
                                <div class="product_price">
                                    <?= $dataExtraBann ?>  <?= $dataExtraBannSol ?>
                                    <div class="on_sale">
                                ...
                                <br>
                                <div class="product_price">
                                    <?= $dataExtraBann5Pc ?><?= $dataExtraBannSol5Pc ?>
                                    <div class="on_sale">
                                        <span>Impuestos Incluidos</span>
                                    </div>
                                    <br>
                                            ...
                                            <div class="product_action_box">
                                                <ul class="list_none pr_action_btn">
                                                    <li class="add-to-cart">
                                                        <a onclick="CARRITO.espe_prod_carr(<?= $rowPR['prod_id'] ?>)" href="javascript:void(0)"><i class="icon-basket-loaded"></i> Añadir al carrito</a></li>
